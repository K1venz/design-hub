...
用法：
  ...
  prod 公网：PROD_BASE=https://203.0.113.10 API_PREFIX=/api uv run python ../image-qa/world_a_removal_regression.py
"""
...
async def main() -> None:
    if not BASE:
        raise SystemExit("✋ 未设 QA_BASE/PROD_BASE（qa 容器 http://localhost:8444 或 prod 公网 https://203.0.113.10 + API_PREFIX=/api）。")
