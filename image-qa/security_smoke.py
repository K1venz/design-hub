...
⚠️ 必须打**公网 nginx 入口** `https://203.0.113.10`（curl -k 自签）——不走 8445 隧道
...
用法：PUBLIC_BASE=https://203.0.113.10 uv run python ../image-qa/security_smoke.py
"""
...
BASE = os.environ.get("PUBLIC_BASE", "https://203.0.113.10").rstrip("/")
# 根族 + /api/ 族（#608：两族都要堵）
BLOCK_PATHS = [
