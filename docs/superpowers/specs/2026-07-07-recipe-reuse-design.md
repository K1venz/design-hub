...
- UI（用户 07-07 细化拍板）：展示卡**双按钮**——**「做同款」+「查看详情」**。
  - **做同款**：原逻辑，未登录 → 登录墙 → 回跳携带配方 → `/set` 预填直达生成。
  - **查看详情**：卡片式弹层（大图 + 配方全项），**内容与 /set 生成界面配置项一一对应**（品类/图型配比/比例/风格描述/平台/地区语言 modifiers——生成界面有的都要有），布局可与生成界面不同但内容必须齐；弹层内再放一个「做同款」CTA。**查看详情无需登录**（recipe 本在公开响应里，看饱了点做同款才拦登录墙）。
