...
日期：2026-08-10  
状态：已批准，待实施  
适用环境：`203.0.113.10` 上的 `/opt/docker/design-hub`
...
正向和反向必须一致：`smtp.image.sepaitech.com` 解析到 `203.0.113.10`，该 IP 的 PTR 再返回 `smtp.image.sepaitech.com`。
