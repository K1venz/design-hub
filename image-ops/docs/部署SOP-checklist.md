...
5. **验证**
   ...
   - **前端有改动**：prod `index.html` 引用的 bundle hash 已变 + 新 bundle `grep` 到新特征串；公网 `curl -sk https://14.103.51.191/ | grep index-*.js` 确认。
   - 公网 `https://14.103.51.191`：`/` 200；加固面 `/docs`·`/api/docs`·`/metrics` 仍 404（未被本次部署破坏）。
