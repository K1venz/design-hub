...
> **✅ 2026-07-08 全屏品牌 Hero 首屏已交付（ISSUE-0061，纯前端·用户实机确认「效果很好」·随 Hero 波部署）**：⚠️ **用户直接修订规格**——**Hero 独立成 index（`/`）、原首页（对话/工具/成果区）移 `/home`**（SideNav/登录跳转已同步；**非**「叠首页下移一屏」旧转述口径）。两段式：第一屏=胶囊公告条「✨全新上线：帮我设计」+ 巨型标题框「一整套电商图，一句话的事。」（四角+装饰+radial 渐隐 + 绿点 ping「内测免费开放中」）撑满整屏；滚下=副标「实朴·电商图片工作站」+描述+双 CTA（主「开始创作」→ 对话输入·未登录走登录墙 / 副「看看实朴出的图」→ 暂 `/home` 成果区、**专门案例页待用户明确**）。**鼠标跟随彩带光轨**（canvas 80 条弹簧丝带、**shadcn 浅色底**·lineWidth=10·dampening=0.025 1:1 照原参考件、TS 化重写零新依赖、touchmove passive 不劫持滚动、reduced-motion 兜底）。
