...
> **✅ 2026-07-08 index 第二波已上线 prod（navbar/footer/logo/字体，`b8be45d`+`d385c2a`+`39ba445`、bundle index-DX2yLlvB、用户实机验收「效果不错」）**：`IndexNavbar`（navbar1 版式 + 产品功能/帮助 hover 下拉 + 登录态感知右钮 + 移动端汉堡面板 + 用户新**六边形 logo**）+ 标题 **wave 逐字艺术字**（方正风雅体·未装回退衬线）+ `IndexFooter`（**联系方式 + ICP 备案号均占位**、页边距按用户反馈调）。**⚠️ 两待用户后补项**：① **方正风雅体授权字体文件（woff2）**到位 frontend-b 打包；② **联系方式 / ICP 备案号真值**（联动 §7.A 备案、忘记密码联系方式）到位替换占位。
