...
> **🔄 改版：「高度复刻」→「完全复刻」（2026-06-15 · 用户已签设计，⏸️ **HOLD 待用户自测**·ISSUE-0062）**。Spec=`docs/superpowers/specs/2026-06-15-clone-full-replicate-redesign-design.md`。**⏸️ 状态纠正（2026-07-08）**：卡已改版（`d0b9d66` 2026-06-22），但**用户点名 HOLD 等他自测**、码故意未同步→`test_clone_blocks_match_card` 红=改版收尾前哨兵（团队基线记「+1 known WIP red」、prod 走「高度复刻」正常、非 bug）。**用户自测拍方向后**才收尾实现（含 clone_mode 重命名存量行→用户签字）或弃改版。详见 ISSUE-0062。**动因**（QA admin 实测坐实 #842）：原「高度复刻」两个不足——① 风格贴不彻底 ② 参考图文案被**整体排除**、给不了爆款大字卖点角标。新档「完全复刻」= **三贴一隔**：① 画面风格完全复制（构图/光影/配色全贴参考图）② **文字样式复刻**（图上字按参考图的字体/排版样式呈现）③ 文案走**用户 overlay_texts**（verbatim、选填、复用卖点图机制原样 2 条×12 字，用户拍死不放宽 #844）④ 参考图原文案**零泄漏**（绝不照抄、无 overlay 不自编、留空只复刻画面不上字）⑤ 产品保真不变。
...
> **✅ 2026-07-08 全屏品牌 Hero 首屏已上线 prod（ISSUE-0061）**：**Hero 独立成 index（`/`）、原首页（对话/工具/成果区）移 `/home`**（SideNav/登录跳转已同步）。**⚠️ 设计定版=MarqueeHero 版**（用户 2026-07-08 给 21st.dev hero-3 参考件、对比初版彩带光轨后拍板「这个更好看」替换、初版退役删除）：**白底 + 文字栈逐词 reveal + 描述半叠走马灯 + CTA 浮图上 + 底部实拍图无限走马灯**（渐隐 mask·卡片旋转错落·12 张实朴真实出图 512px jpeg 打包 assets 784KB·**零 API 依赖**）；**单 CTA「开始创作」→/home**（未登录走登录墙）。用户实机确认「更好看」。（初版彩带光轨 index-JLOHJ2ij 先上、MarqueeHero d7c22a6 待纯前端波部署=终版。）
> **✅ 2026-07-08 index 第二波已上线 prod（navbar/footer/logo/字体，`b8be45d`+`d385c2a`+`39ba445`、bundle index-DX2yLlvB、用户实机验收「效果不错」）**：`IndexNavbar`（navbar1 版式 + 产品功能/帮助 hover 下拉 + 登录态感知右钮 + 移动端汉堡面板 + 用户新**六边形 logo**）+ 标题 **wave 逐字艺术字**（方正风雅体·未装回退衬线）+ `IndexFooter`（**联系方式 + ICP 备案号均占位**、页边距按用户反馈调）。**⚠️ 待用户后补项**：① **方正风雅体授权字体文件（woff2）**到位 frontend-b 打包；② **联系方式真值**（联动忘记密码联系方式）到位替换占位。**✅ ICP 备案号已下发**（2026-07-09 `浙ICP备2026024031号-1`，见 §7.A）→ frontend-b 法定挂载执行中（footer 替占位+超链工信部+显著位）。
...
#### 3.16 「配置大模型」页面 —— 管理员全局配 + 备用渠道切换（需求 · 2026-07-07，ISSUE-0057）✅ MVP 已上线 prod（2026-07-08·备用渠道切换=治 0056 结构性解 live·待用户 admin 复核）
...
#### 3.18 商品品类扩展（需求 · 2026-07-08，ISSUE-0060）✅ 已上线 prod 全收口（五品类全链·真图特性绿·精选入成果区·知识库 flip·收官 smoke 三条全绿）
> **✅ 品类真图验收（2026-07-08 异步管线）**：四品类特性全生效——服装织物垂坠/鞋侧45°麂皮/美妆材质光感/数码冷调科技感，**不再「万物皆花生」**。**⚠️ 已知限界=密集小字保真**：品牌大字/标题保真好（SENYANJI/SAMSUNG 全对），但**密集中文小字（成分表/参数小字）可能漂移**=**模型能力边界、非缺陷**；产品口径「大字保真、密集小字建议后期贴字」已入知识库+文档（防 chat over-promise）。精选 3 张 text-safe（FASHION/SHOES/DIGITAL）入成果区，**美妆暂缓**（真品牌密集小字错字风险）。知识库品类行已 flip「支持 5 品类」。
...
- **④ 验收（QA，⚠️ 真图依赖 key）**：5 品类各真出 1 套（n=3）看特性生效（不再「万物皆花生」·各品类真实品牌文字保真·≈¥6 key 恢复后跑）· 卡闸 pytest 逐字绿 · 前端选择器/chat 品类/配方带品类 · FOOD 全链零回归 · **品类精选图入成果区正常展示**（B5）。代码/卡先落地、真图排 key 后。
- **⑤ 品类精选图入首页成果区（用户 #1035 追加，获客面更宽）**：素材=`image-qa/品类扩展素材/{服装,美妆,鞋类,数码}/` 各 1 张高清真实产品图（带真实品牌文字）；流程扩=key 恢复→5 品类各 1 套→质检评图→**精选入桶 showcase/ + ShowcaseEntry+recipe 回填（沿 0053 静态清单机制、dev 落码）**→首页成果区（§3.14 ④）从纯食品扩**多品类**；每品类精选 1–2 张、总量控制（现 13+新 4–8）；口径沿 0053（无用户数据/无鉴权/配方不露内部 prompt）、沿 0054 栅格+懒加载自然容纳。
...
- **🎉 里程碑（2026-07-09，coordinator #1131）：ICP 备案号下发 = `浙ICP备2026024031号-1`**（session-long 头号硬杆达成）。**✅ 法定挂载完成**（frontend-b `c55d7d4`+`09f5733`，待部署）：① 备案号真值替换所有 footer 占位；② 超链 `https://beian.miit.gov.cn`（target=_blank+rel=noopener·两页实测）；③ 显著可读（HomeFooter 原 11.5px faint 淡色=用户点名「灰到看不见」反例→提升 12.5px 实色 font-medium、IndexFooter neutral-700 13.5px）。**同波 footer 统一**：抽 `components/layout/SiteFooter` **单一事实源**（IndexFooter 复用到 /home[退役 HomeFooter]+/terms+/privacy；墙内操作页不挂=工作密度优先）。Playwright 四页实证。→ 纯前端波部署 + push 远程 dev（新规首跑）。**连带解锁**：④ 域名 `image.sepaitech.com` 443 启用（接入备案生效/边界放行后，ops 跟：域名默认入口+HSTS）；⑤ **§3.17 登录密码加密根治的「正式域名+LE 证书」前提就位**（备案号→域名可正式启用→LE 已签）；⑥ toC 全量放开的 §7.A gate 达成（**§7.B 内容安全仍前置**、两条线别混）。
...
| # | 项 | 级别 | 拍板结果 | 周期预估 |
|---|---|---|---|---|
| 7.A | 域名+TLS+备案 | 🟢 | 🎉 **备案下发（2026-07-09）**：ICP 备案号 `浙ICP备2026024031号-1`（域名 `image.sepaitech.com` + LE 证书早已就位）→ footer 法定挂载执行中（frontend-b）+ 域名 443 启用待接入备案生效（ops 跟）。**头号硬杆达成**、解锁密码根治正式域名前提 + toC 7.A gate | 挂载纯前端波 |
