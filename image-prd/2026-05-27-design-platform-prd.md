...
> **🔨 2026-07-08 增补：全屏品牌 Hero 首屏（ISSUE-0061，纯前端）**：`/` 前加 **100vh 全屏 Hero 区**（顶部胶囊公告条「✨全新上线：帮我设计」+ 巨型标题框「一整套电商图，一句话的事。」四角+装饰+radial 渐隐 + 绿点 ping「内测免费开放中」+ 副标「实朴·电商图片工作站」+ 双 CTA：主「开始创作」滚对话输入·未登录走登录墙 / 副「看看实朴出的图」滚成果区）+ **鼠标跟随彩带光轨动效**（canvas 80 条弹簧丝带、色相贴品牌紫、TS 化重写零新依赖、移动端不劫持滚动、reduced-motion 兜底）。现有 Hero/工具区/成果区**整体下移一屏**、路由/SideNav 不动。
