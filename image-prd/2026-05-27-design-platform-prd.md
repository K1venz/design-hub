...
- **🎉 里程碑（2026-07-09，coordinator #1131）：ICP 备案号下发 = `浙ICP备2026024031号-1`**（session-long 头号硬杆达成）。**法定挂载立即执行**（frontend-b）：① 替换所有 footer 备案占位（IndexFooter `/` + `/home` 工作首页 footer + 协议页独立 footer）；② **超链接 `https://beian.miit.gov.cn`**（工信部备案系统，target=_blank + rel=noopener）；③ **显著位置**（页脚居中/明显、对比度可读=《备案管理办法》硬要求，非灰装饰字）。→ 纯前端波部署 + push 到远程 dev（新规矩首跑）。**连带解锁**：④ 域名 `image.sepaitech.com` 443 启用（接入备案生效/边界放行后，ops 跟：域名默认入口+HSTS）；⑤ **§3.17 登录密码加密根治的「正式域名+LE 证书」前提就位**（备案号→域名可正式启用→LE 已签）；⑥ toC 全量放开的 §7.A gate 达成（**§7.B 内容安全仍前置**、两条线别混）。
