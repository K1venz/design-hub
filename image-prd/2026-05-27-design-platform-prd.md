...
- **现状**：IP 直连 `https://203.0.113.10` + **自签证书**（CN=design-hub.local）→ 浏览器全屏红色告警，toC 用户第一步就流失；国内服务器对公众提供 web 服务**法定需 ICP 备案**。
- **建议**：注册正式域名（与 7.G 产品名联动）→ ICP 备案（**周期 1~2 周+，全清单最长杆，建议最先启动**）→ CA 证书（Let's Encrypt 免费即可）。
- **🔄 进展实锤（2026-06-15，ops #857，已从抽象转真实卡点）**：用户已拿到子域名 **`image.sepaitech.com`**，ops 已给签正规 Let's Encrypt 证书（证书 OK）。但**外网 443 被边界按 SNI 秒断（TLS 握手 29ms 拒）= `sepaitech.com` 未完成 ICP 备案/接入备案**（大陆服务器法定要求，边界拦在服务之前）——**非故障**：服务器/出图/数据全健康、IP `https://203.0.113.10` 与域名 80 正常，**只域名 443 被备案拦**。→ **正解只有用户办 ICP 备案 + 火山接入备案**（ops 办不了、只能等）；**临时让美工走 IP** `https://203.0.113.10`。这是当前 toC 全量公开的**头号实际卡点**（也呼应交付美工软发布=暂走 IP）。运维侧待办（备案下来后）：域名默认入口 + HSTS 对齐（ops 可做）。
