...
① **知识环**：问「复刻怎么用/出图多少钱/历史在哪」答案与知识库一致；问知识库外（「能出视频吗」）明说不支持不编造。② **顾问环**：问「卖点文案怎么写好」给建设性建议、不再拒。③ **核心环零回归**：三条真路径 + 费用闸 + 0051 持久化不变。④ **安全地板**：违规话题仍拒。⑤ **长会话**：30+ 轮上下文裁剪生效且回答连贯。⑥ **工具环（新增）**：「我最近出过什么图」→`query_my_jobs` 返真数据；「用上次配置再出一套」→取配方回填→**费用确认卡**（不确认不出图=护栏①）；**「出图多少钱/我还有多少额度」→`get_pricing_quota` 答 model_config 实时值**（管理员改价即变、非背固定数字=波动性原则）；读工具**越权面 owner 隔离**（只见自己=护栏③）。
...
- 2026-07-08 [PM] **波动性信息原则入档（用户拍板，coordinator #1043）——「知识库管稳定、工具管波动」完整分层**：
