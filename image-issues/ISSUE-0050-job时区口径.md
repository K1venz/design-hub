...
- 2026-07-08 [PM] **队列后移（coordinator #1044，0057 提级）**：ISSUE-0057「配置大模型」被用户点名提级到本条之前 → **新队列 = A/B 线收口 → 0057 → 0050 时区批（+0055 同批）**。0050 仍 P3、非阻断、随 0055 同批（都 image-code 内），排在 0057 之后由 coordinator 编排 dev+frontend-b 协调档期。key 恢复则插入即跑收口三连（0052/0055/0056），本条不抢那档。owner=coordinator。
