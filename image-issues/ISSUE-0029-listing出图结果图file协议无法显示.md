...
- 2026-06-05 [运维] **(a) 完成——按 GitHub main 重新部署 prod**：
  · 注入服务器 `.env` `IMAGE_PUBLIC_BASE_URL=https://203.0.113.10`（chmod 600，不入库）
  · CI(`workflow_dispatch`, ref main=3325615 含后端 7df59bb) 重建 api 镜像并重启
  · 验证：容器内 `Settings().image_public_base_url == 'https://203.0.113.10'`（新代码+env 双生效）、api healthy、`/img/<sha>.png`→200、站点 200
  · **prod 端到端就绪**：新出图 url=`https://203.0.113.10/img/<sha>.png` → nginx 200 → 前端 `<img>` 可显示
