...
- ⚠️ **线上（14.103.51.191）同样坏且更严重**：prod 无 `/__localimg`，file:// 在任何浏览器都加载不了 → listing 结果永远裂图。
...
- 2026-06-04 [用户/QA] **方案拍板：nginx 静态反代图片目录 + 后端返「尽量绝对」url，暂不鉴权（图无安全顾虑）。**
  ...
  **① 后端（开发，image-code）——把出图 url 从 file:// 改成 web 绝对地址：**
  ...
  - `.env`：prod `IMAGE_PUBLIC_BASE_URL=https://14.103.51.191`；dev `http://localhost:3000`。
  ...
  **QA 验收（我）**：① 真服务器 `curl https://14.103.51.191/img/<name>.png` → 200 image/png；
...
- 2026-06-04 [运维] **②(Ops/nginx) 已完成并部署到生产**：
  ...
  · `nginx -t` 通过、重建生效；实测 `curl https://14.103.51.191/img/14ca4f72bb087e83.png` → **200 image/png**（QA 验收①先行达成），`/img/`(目录) → 403 防遍历。
  · 已合入 .github CI（push main 会带上），并直接部署生效。
  **仍待**：① 后端返 `…/img/<sha>.png`（开发，image-code）+ ③ dev 一致性（前端）。
  其中 prod `IMAGE_PUBLIC_BASE_URL=https://14.103.51.191` 这条 env 由运维在①落地后注入服务器 .env（我的域，待开发加好 settings 字段即注入）。owner 维持=开发。
