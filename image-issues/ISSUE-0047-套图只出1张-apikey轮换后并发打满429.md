...
status: 已确认        # dev 读码坐实根因（#852）；代码未动，等用户「继续」才碰 prod（真实用户在用）
...
- 2026-06-15 [PM] 交付美工后**首个真实用户 bug**，PM 分诊归档（承 #839 真实用户保障姿态）：定级 **P1**
  ...
  **待用户放行**：修复碰 prod、dev 在等「继续」——PM 已上报用户（非安全非 DB，可 coordinator 代放行，
  但真实用户在用建议用户知会）。dev 放行后走状态机：修复中 → 待验证(QA qa 先行+prod 复测) → 已修复 → 关闭。
