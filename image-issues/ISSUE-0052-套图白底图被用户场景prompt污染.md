...
## ⚠️ 待 PM 定产品口径（prompt/dev 会诊输入 → PM 拍）
**白底图型对用户文本的处置粒度**——两档：
- **档 A（倾向）**：白底图型**完全剥离用户自由文本**（用户 styling 只作用于场景图/卖点图；白底=纯白主图合规向、不吃场景），最干净、根治污染。
- **档 B**：只剥离/降权「场景类」文本、保留「产品强调类」文本（如「突出金色包装」）——更细但需判别文本类别、复杂度高、易漏。
PM 倾向 A（白底图语义本就排斥场景、YAGNI 不做文本分类）；prompt/dev 会诊给可行性 + 卡措辞，PM 拍板后落。
